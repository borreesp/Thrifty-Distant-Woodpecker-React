...
type SkillItem = { name: string; progress: number };

export const EquipmentSkills: React.FC<{ gear: GearItem[]; skills: SkillItem[] }> = ({ gear, skills }) => {
  return (
          ...
          {skills.map((s) => (
            <div key={s.name} className="rounded-lg bg-white/5 px-3 py-2 ring-1 ring-white/5">
              <div className="flex items-center justify-between">
                <span>{s.name}</span>
                <span className="text-cyan-200">{s.progress}%</span>
              </div>
